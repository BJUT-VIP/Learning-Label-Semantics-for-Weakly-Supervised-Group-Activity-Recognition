...
parser.add_argument('--data_path', default='/data2/wq/videos/', type=str, help='data path')#/root/autodl-tmp/Datasets/Volleyball_dataset/videos/
...
parser.add_argument('--epochs', default=60, type=int, help='Max epochs')
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # test_log = validate(test_loader, model, criterion, epoch, memory, device)
        train_log = train(train_loader, model, criterion, optimizer, epoch, memory,device)
        ...
        if epoch % args.test_freq == 0 and epoch >= 5:
...
def train(train_loader, model, criterion, optimizer, epoch, memory,device):
    ...
    for i, (_, images, activities, ret) in tqdm(enumerate(train_loader)):
        ...
        # compute output
        scr, score,src_mod = model(images,mod=True)     # [B, C]
        ret[0] += i*args.batch+torch.tensor(range(num_batch)).cuda()
        logits = memory(scr, ret, epoch)
        if epoch > 5:

            multilabel = labelpred.predict(memory.mem.detach().clone(), ret[0].detach().clone())
            loss1 = criterion1(logits, multilabel, True)
            label_loss = F.binary_cross_entropy(src_mod, multilabel)
        else:
            loss1 = criterion1(logits, ret[0])
...
@torch.no_grad()
def validate(test_loader, model, criterion, epoch, memory,device):
    ...
    for i, (_, images, activities, ret) in tqdm(enumerate(test_loader)):
        images = images.cuda()##################################
        ...
        scr, score,src_mod = model(images,mod=True)  # [B, C]
        ret[0] += i * args.test_batch + torch.tensor(range(num_batch)).cuda()
        logits = memory(scr, ret, epoch)
        multilabel = labelpred.predict(memory.mem.detach().clone(), ret[0].detach().clone())
        loss1 = criterion1(logits, multilabel, True)
