...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_decoder_layers=6,num_encoder_layers=6,
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        # src = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed)
        src, att = self.decoder(src, src, memory_key_padding_mask=mask,
                                pos=pos_embed, query_pos=src)  ################ 92.37
        src = src.squeeze(0)
        ...
        return hs.transpose(1, 2), src# att
...
class Transformer_v2(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        hs, att = self.decoder(tgt, query_embed, memory_key_padding_mask=mask,
                               query_pos=src)

        return hs.squeeze(0), src# att hs.transpose(1, 2),

class TransformerEncodertra(nn.Module):
...
class TransformerEncoderLayertra(nn.Module):

    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False):
        super().__init__()
        # self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.Tra_attn = TrajectoryAttention(d_model, nhead, qkv_bias=False, attn_drop=0., proj_drop=0.,
    ...
    def forward_post(self,
        ...
        E=1
        src2,_ = self.Tra_attn(q, k, src,E, attn_mask=src_mask,
                              key_padding_mask=src_key_padding_mask)#[0]
        # tgt2, att = self.multihead_attn(query, key, value, attn_mask=memory_mask,
...
def build_token_encoder_v2(args):
...
class TrajectoryAttention(nn.Module):
    ...
    def forward(self, q, k, x,E, attn_mask=True,
        ...
        k = k.permute(1, 0, 2).reshape(-1, F * P, C)
        x = x.permute(1, 0, 2).reshape(-1, F * P, C)
        if E == 0:
            q = q.permute(1, 0, 2).reshape(-1, F * 6, C)
        else:
            q = q.permute(1, 0, 2).reshape(-1, F * P, C)
        B, N, C = x.shape
        ...
        if E == 0:
            ...
            x = x.reshape(-1, F, P, C).permute(2, 0, 1, 3).reshape(P, -1, C)
        # x = x.reshape(-1, F, 6, C).permute(2, 0, 1, 3).reshape(6, -1, C)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.fc_final = nn.Linear(6*256, 256)
    ...
    def encode_text(self, text):
        ...
        x = x + self.positional_embedding#######92.5
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)###########92.5
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.fc_final(x.reshape(8,-1))#(6,-1)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt = tgt + self.dropout1(tgt2)
        tgt = self.norm1(tgt)
        att = torch.zeros([2,5],dtype=torch.float)
        # tgt2, att = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos),
        #                                 key=self.with_pos_embed(memory, pos),
        #                                 value=memory, attn_mask=memory_mask,
        #                                 key_padding_mask=memory_key_padding_mask)
