...
parser.add_argument('--epochs', default=34, type=int, help='Max epochs')
...
parser.add_argument('--max_lr', default=7.5e-5, type=float, help='Max learning rate')
...
#/data/yz/tm-detectorfree/result/DF_basemodel_2/epoch26_92.00%.pth
...
def train(train_loader, model, criterion, optimizer, epoch, memory,device):
    ...
    for i, (_, images, activities, label_in) in tqdm(enumerate(train_loader)):
...
def validate(test_loader, model, criterion, epoch, memory,device):
    ...
    for i, (_, images, activities, label_in) in tqdm(enumerate(test_loader)):
        images = images.cuda()##################################
